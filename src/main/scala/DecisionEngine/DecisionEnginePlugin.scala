...
trait DecisionEnginePlugin {
  ...
  def learn(data: Vector[DataWrapper], model: Option[DataModel]): DataModel //model is optional (if passed, the model is further trained with additional training examples
