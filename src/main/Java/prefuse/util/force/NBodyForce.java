        ...
        try {
            ...
            if ((!n.hasChildren) && !(n.value != null)) {
                ...
                if (isSameLocation(n.value, p)) {
                    ...
                    n.value = null;
                    //insertHelper(v,n,x1,y1,x2,y2);
                    float x = v.location[0], y = v.location[1];
                    float splitx = (x1 + x2) / 2;
                    float splity = (y1 + y2) / 2;
                    int i = (x >= splitx ? 1 : 0) + (y >= splity ? 2 : 0);
                    // create new child node, if necessary
                    ...
                    // recurse
                    insert(v, n.children[i], x1, y1, x2, y2);

                    // insertHelper(p,n,x1,y1,x2,y2);

                    float xN = p.location[0], yN = p.location[1];
                    float splitxN = (x1 + x2) / 2;
                    float splityN = (y1 + y2) / 2;
                    int iN = (xN >= splitxN ? 1 : 0) + (yN >= splityN ? 2 : 0);
                    // create new child node, if necessary
                    if (n.children[iN] == null) {
                        n.children[iN] = factory.getQuadTreeNode();
                        n.hasChildren = true;
                    }
                    // update bounds
                    if (iN == 1 || iN == 3) x1 = splitxN;
                    else x2 = splitxN;
                    if (iN > 1) y1 = splityN;
                    else y2 = splityN;
                    // recurse
                    insert(p, n.children[iN], x1, y1, x2, y2);
                }
            }
