...
class TreeTest extends FunSuite{
  ...
  test("Node updatePredictions is none before events are added"){
    val n = new Node[Int, Int](1,1)
    n.setKey(2)
    assert(n.getPredictions.size == 0)
  }
  ...
  test("Node predictions are correct"){
    ...
    n.setKey(2)
    n.updateEvents(666)
    assert(n.getPredictions.get(666).get == 1.0)
    n.updateEvents(666)
    assert(n.getPredictions.get(666).get == 1.0)
    n.updateEvents(777)
    assert(n.getPredictions.get(777).get == 1.0/3)
    n.updateEvents(777)
    assert(n.getPredictions.get(777).get == 2.0/4)
    n.updateEvents(888)
    assert(n.getPredictions.get(666).get == 2.0/5)
    assert(n.getPredictions.get(777).get == 2.0/5)
    assert(n.getPredictions.get(888).get == 1.0/5)
    n.updateEvents(999)
    assert(n.getPredictions.get(666).get == 2.0/6)
    assert(n.getPredictions.get(777).get == 2.0/6)
    assert(n.getPredictions.get(888).get == 1.0/6)
    assert(n.getPredictions.get(999).get == 1.0/6)
  }
  ...
  test("Node getProbability for String events after multiple updates. (String key)"){
    val n = new Node[String, String](1,1)
